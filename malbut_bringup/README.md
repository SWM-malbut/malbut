...
- Nav2 기본 설정: `navigation/config/nav2_params.yaml`
...
`nav2_params_file`을 지정한다. 제조사 원본을 복사하거나 패치하지 않는다.
...
기본 연결값은 ROSOrin/Aurora 계열의 시작 설정이며, **이 로봇에서 아직
실측 확정한 값은 아니다.** `ros2 topic list -t`와 센서 Header를 확인하고
실제 값이 다르면 아래 인자를 변경한다. 이름을 바꾸는 relay는 만들지 않는다.
