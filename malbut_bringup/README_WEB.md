...
5. 위치와 준비 상태를 확인한 뒤 사람 추적·순찰을 요청한다.
...
- 실시간 2D 지도는 `/map`, 로봇 위치·방향은 TF에서 읽는다. 지도 원점의 회전을
  반영하며 지도 이미지는 변경될 때만 내려받고 위치 정보는 약 1초마다 갱신한다.
  현재 수신이 끊기면 마지막 지도를 비활성 상태로 표시한다.
- 지도 선택·클릭은 이동이나 위치 초기화를 하지 않는다. 최초 실행이거나 전원이
  꺼진 동안 로봇을 옮겼다면 RViz의 **2D Pose Estimate**로 실제 위치를 지정한다.
