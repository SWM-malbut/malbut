...
def _setup(context):
    ...
    if navigating and navigation:
        ...
        navigation_path = (
            _file(value('navigation_launch_file'), 'navigation launch')
            if value('navigation_launch_file') else _package_file(
    ...
    if perception:
        options = {name: value(name) for name in (
            ...
            'python_executable', 'device', 'reid_model_path',
...
def generate_launch_description():
    ...
    defaults = {
        ...
        'map': '',
        'nav2_params_file': '',
        # Override these with the actual vendor topics. No topic relay or
        # synthetic optical transform is introduced by Bringup.
