...
배포 소스는 `malbut_test/homecam_web`입니다. 개발은 원본에서 하고 배포 복사본에도 반영합니다.
...
- 로봇이 보고하는 `malbut_manager_v1` 프로필에서는 현재 기능만 표시합니다.
  예전 Gazebo의 배회·일시정지·방 편집 API를 실기기에 요청하지 않습니다.
- 명령 큐의 `completed`는 로봇 측 **접수 처리 완료**입니다. 미션 성공·실패·취소는
  로봇이 올리는 `target.requests`의 최종 상태·결과를 봅니다.
