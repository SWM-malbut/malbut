...
export function parseRobotCommand(value: unknown): {
  ...
  if (operation === "runtime_stop" || operation === "mission_cancel") {
    return Object.keys(payload).length === 0 ? { operation, payload } : null;
  }
  if (operation === "runtime_start") {
    ...
    return payload.mode === "navigation" && Object.keys(payload).length === 2 &&
      typeof payload.map === "string" && /^[A-Za-z0-9][A-Za-z0-9_-]{0,63}\.ya?ml$/.test(payload.map)
