...
실기기 빌드와 Bringup은 상위 `malbut_test/README.md`를 따릅니다.
Bringup에서 순찰 서버를 실행했다면 아래 명령으로 중복 실행하지 않습니다.
...
ros2 launch malbut_patrol patrol.launch.py use_sim_time:=false \
  camera_image_topic:=/depth_cam/rgb0/image_raw \
  camera_info_topic:=/depth_cam/rgb0/camera_info
```

표준 광학 프레임 헤더를 제공하면 `camera_optical_frame`을 비워둡니다.
실제 센서 프레임과 TF 연결은 로봇에서 확인해야 합니다. 방 라벨은 선택 사항이며
