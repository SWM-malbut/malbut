...
async function assertDriveCommandAllowed(input: {
  ...
  if (nav2.robot_interface === "malbut_manager_v1") {
    // Readiness, resources and preemption belong to the current robot manager.
    // Runtime/cancel requests must work even while localization is unavailable.
    return;
  }
  if (managedOperation) throw new Error("UNSUPPORTED_ROBOT_COMMAND");
  ...
  await getD1()
    .prepare(
      `UPDATE robot_commands
       ...
       WHERE device_id = ? AND status = 'queued' AND operation = 'manual_move'
         AND requested_at < ?`,
    )
    .bind(
      new Date().toISOString(),
      deviceId,
      new Date(Date.now() - MANUAL_MOVE_QUEUE_MS).toISOString(),
    )
    .run();
  await getD1()
    .prepare(
...
export async function listRobotCommands(deviceId: string, userEmail: string, limit = 20) {
  ...
  return result.results.map((row) => {
    ...
    const size = row.result_json ? Buffer.byteLength(row.result_json, "utf8") : 0;
    return size > 4 * 1024 ? { ...command, result: { truncated: true, bytes: size } } : command;
  });
}
