...
export function RobotMapPanel({
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      if (command && command.id !== processedCommand.current && ["completed", "failed"].includes(command.status)) {
        ...
        } else if (command.operation === "map_delete") {
          setNotice("저장 지도를 삭제했습니다.");
        } else if (command.operation === "zones_save") {
          setNotice("구역을 로봇에 저장했습니다. 주행에 반영하는 중입니다.");
        } else if (command.operation === "robot_ping") {
          setNotice("로봇이 응답했습니다. 디버깅에서 왕복 시간을 확인하세요.");
  ...
  const workspace = useManagedRobotWorkspace(deviceId, snapshot, managed);
  const currentManagedGoal = workspace.goal;
  ...
  const visibleGoal = managed ? currentManagedGoal : isRecord(navigationPreview?.resolved)
          ...
          {managed ? (
            snapshot?.map ? (
              <ManagedRobotMap
                ...
                initialPose={workspace.pose}
                zones={workspace.zones.zones}
                drawing={workspace.zones.drawing}
                selectedZone={workspace.zones.selected}
                tool={mapMode === "navigate" || mapMode === "pose" || mapMode === "zones" ? mapMode : "view"}
                interactive={Boolean(isOwner && snapshot.online && runtimeMode === "navigation")}
                onPick={(point) => {
                  if (mapMode === "zones") {
                    workspace.zones.pick(point);
                    return;
                  }
                  workspace.selectGoal(point);
                  setNotice("위치를 선택했습니다. 이동 버튼을 누르면 주행을 시작합니다.");
                }}
                onPose={(pose) => {
                  workspace.selectPose(pose);
                  setNotice("현재 위치를 지정했습니다. 위치 보정에서 '이 위치로 설정'을 누르세요.");
                }}
              />
            ) : (
