...
const MAP_FILE = /^[A-Za-z0-9][A-Za-z0-9_-]{0,63}\.ya?ml$/;
...
export function parseRobotCommand(value: unknown): {
  ...
  if (["runtime_stop", "mission_cancel", "robot_ping", "robot_diagnostics"].includes(String(operation))) {
    return Object.keys(payload).length === 0
      ? { operation: operation as RobotOperation, payload }
      : null;
  }
  if (operation === "map_delete") {
    return Object.keys(payload).length === 1 && typeof payload.map === "string" && MAP_FILE.test(payload.map)
  ...
  if (operation === "zones_save") {
    return Object.keys(payload).length === 2 && typeof payload.map === "string" &&
      MAP_FILE.test(payload.map) && validZones(payload.zones)
      ? { operation, payload } : null;
  }
  if (operation === "debug_mission_start") {
    // Any registered capability: the robot's manager validates the fields.
    return Object.keys(payload).length === 2 && typeof payload.capability === "string" &&
      /^[a-z][a-z0-9_]{0,63}$/.test(payload.capability) && isObject(payload.arguments) &&
      Buffer.byteLength(JSON.stringify(payload.arguments), "utf8") <= 8 * 1024
      ? { operation, payload } : null;
  }
  if (operation === "runtime_start") {
    if (payload.mode === "mapping" && Object.keys(payload).length === 1) {
      return { operation, payload };
    }
    return payload.mode === "navigation" && Object.keys(payload).length === 2 &&
      typeof payload.map === "string" && MAP_FILE.test(payload.map)
      ? { operation, payload } : null;
  }
  if (operation === "mission_start") {
    return Object.keys(payload).length === 2 && validMission(payload.capability, payload.arguments)
      ? { operation, payload } : null;
  }
...
/** Only expose existing Malbut capabilities; no arbitrary ROS endpoint or YAML. */
function validMission(capability: unknown, value: unknown): boolean {
  ...
  if (capability === "patrol") {
    return keys.length === 1 && integerInRange(value.thoroughness, 0, 2);
  }
  if (capability === "follow_person") {
    return keys.length === 3 && integerInRange(value.target_mode, 0, 1) &&
      typeof value.target_person_id === "string" && value.target_person_id.length <= 128 &&
      (value.target_mode === 0 || value.target_person_id.trim().length > 0) &&
      finiteNumber(value.desired_distance_m) && value.desired_distance_m >= 0.2;
  }
  if (capability === "navigate_to_pose") {
    return keys.length === 3 && finiteCoordinate(value.x) && finiteCoordinate(value.y) &&
      finiteNumber(value.yaw);
  }
  if (capability === "relocalize") {
    // 0: saved pose first, 2: whole-map search, 1: the operator's map pose.
    if (value.method === 0 || value.method === 2) return keys.length === 1;
    return value.method === 1 && keys.length === 4 && finiteCoordinate(value.x) &&
      finiteCoordinate(value.y) && finiteNumber(value.yaw);
  }
  return false;
}
