...
## 도메인 없이 AWS 기본 HTTPS 주소로 시작

`-c ingressMode=cloudfront -c authMigrationPhase=cleanup`을 지정하면
`https://<distribution>.cloudfront.net` 주소를 사용한다. EC2 서버, Elastic IP,
Route 53 Hosted Zone, 사용자 소유 도메인과 별도 ACM 인증서가 필요하지 않다.
기존 도메인 배포의 기본값은 `ingressMode=custom-domain`으로 유지된다.

이 모드는 `CloudFront HTTPS → VPC origin → private HTTP ALB → ECS Fargate`로
연결한다. ALB는 isolated subnet에 있고 CloudFront origin-facing prefix list만
80번 포트에 접근할 수 있다. 외부에서 ALB의 HTTP 주소로 우회할 수 없다.
현재 로그인은 Cognito 서버 API를 사용하므로 Hosted UI용 도메인이 필요 없다.
기존 ALB 인증 단계와 섞지 않도록 CloudFront 모드는 `cleanup`만 허용한다.
기존 스택의 ingress 변경은 ALB 교체를 포함할 수 있으므로 반드시 diff를
검토한다. 확인하지 않은 기존 계정/스택을 새 배포 대상으로 재사용하지 않는다.

모든 요청의 캐시는 비활성화하고 cookie, Authorization, Origin, query string,
Host를 전달한다. 장치·내부 API의 POST/DELETE 등 모든 메서드를 허용하되,
실제 권한 검증은 기존 앱에서 그대로 수행한다. ALB는 CloudFront가 추가한
마지막 viewer IP를 보존하며, 재생 쿠키는 외부 HTTPS origin을 기준으로
`Secure`를 유지한다. `AUTH_PUBLIC_ORIGIN`, maintenance URL과 `HomecamUrl`
output에는 생성된 CloudFront 주소가 자동 주입된다.

서울 리전은 CloudFront VPC origins를 지원한다. 배포 역할에는 기존 권한 외에
CloudFront distribution/VPC origin 및 해당 service-linked role 생성 권한이
필요하다. VPC origin 생성에는 약 15분까지 걸릴 수 있다.
공식 참고: [VPC origins](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/private-content-vpc-origins.html),
[CloudFront 기본 인증서](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/DownloadDistValuesGeneral.html).

### 미커밋 테스트 복제본을 배포하는 snapshot 모드

원본 수정·검증 후 `malbut_test/malbut_web`를 먼저 동기화한 다음, 이 디렉터리의
...
npm run synth -- \
  -c stage=robot \
  -c 'deviceIds=["jetson-homecam"]' \
  -c ingressMode=cloudfront \
  -c authMigrationPhase=cleanup \
  -c deploymentSourceDirectory=malbut_test/malbut_web \
  --no-lookups --quiet
```

`deploymentSourceDirectory`는 CDK 명령의 현재 디렉터리가 아니라 저장소 루트
기준이다. 명령은 원본 CDK에서도 테스트 복제본 CDK에서도 같은 경로를 받는다.
지정하면 CodeBuild는 해당 S3 asset만 읽으며 ECR 태그를
`snapshot-<DeploymentSourceHash>`로 자동 고정한다. `containerImageTag`와
`GIT_SHA` override는 snapshot 모드에서 사용하지 않는다. source 파일이 달라지면
새 hash/tag가 만들어지며 기존 ECR 태그는 덮어쓰지 않는다.

위 예시는 기존 `MalbutHomecam-dev`와 분리된 `MalbutHomecam-robot`을 대상으로
한다. `stage`를 생략하면 기본값 `dev`가 선택되므로, 실제 배포에서도
`stage=robot`과 동일한 `deviceIds`를 명시한다. 장치 ID는 provisioning manifest와
일치해야 한다.

실제 배포는 확인된 AWS 계정/프로필과 대상 스택으로 다음 순서로 진행한다.

1. 동일 snapshot/context로 `ServiceDesiredCount=0` 배포: ECR과 snapshot asset 준비.
2. output의 `ImageBuilderProjectName`을 별도 환경 변수 override 없이 실행하고
   성공 및 output의 `ContainerImageTag` 존재를 확인.
3. **source를 수정하지 않은 상태에서 동일 context**로
   `ServiceDesiredCount=1` 업데이트 배포. source가 바뀌면 1~2를 다시 수행.
4. output의 `HomecamUrl`에서 로그인·미인증 API 401·장치 등록·영상 E2E 확인.

CloudFront 모드에서는 아래 parameter 표의 `HomecamHostedZoneId`와
`HomecamHostedZoneName` 두 값만 생략한다. owner, VAPID, provisioning 입력은
여전히 필요하다. AWS 계정 인증과 유료 리소스 생성은 별도 승인 후 수행한다.
...
배포 전에 선택한 계정/리전의 기존 스택과 리소스를 조회한다. 이 stack은
애플리케이션 리소스를 import하지 않고 `stage`별로 생성하며, 같은 `stage`로
배포하면 기존 스택을 업데이트한다. 새 환경은 별도 `stage`를 명시하고,
기존 환경 업데이트는 대상과 변경 사항을 확인한 뒤 진행한다.
...
실제 배포 전 AWS 관리자가 대상 계정/스택, `cdk diff`와 예상 비용을
검토해야 한다. 특히 ALB, RDS 및 KVS
