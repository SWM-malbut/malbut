...
이 폴더는 배포 적용본이다. `malbut_web/`을 AWS에 배포하고 나머지 로봇 코드는
...
`malbut_web/infra/cdk/README.md`의 CloudFront 기본 HTTPS 주소 배포 절차를 사용한다.
도메인 구매나 IP 게이트웨이 EC2는 필요 없다. 배포 입력은 이 적용본의 `malbut_web/`이어야 한다.
...
- 시작은 센서·주행 준비 → Manager 준비 → 음성 점검 →
  Agent·TTS 준비 → STT 마이크 시작 순서다. 웹은 음성 준비까지 끝나야 준비 완료로
  표시하며, 그전에는 `음성 모델·마이크 준비 대기`를 표시한다.
- Bringup은 하나다. 지도 작성 모드를 요청하면 지도 없이(SLAM) 켜고, 저장 지도 주행을
  요청하면 그 지도(AMCL)로 켠다. 이미 켜져 있으면 재시작 없이 관리자가 위치 추정만
  바꾸며, 이동 미션이 남아 있으면 거부한다. SLAM과 AMCL은 동시에 켜지 않는다.
...
3. 저장 지도 선택 → 주행 모드 요청(재시작 없이 전환) → 실제 위치 일치 확인.
   맞지 않으면 **위치 다시 찾기** 또는 **현재 위치 지정 → 이 위치로 설정**.
4. 사람 추적·순찰·목적지 이동 요청 → 실행 상태 → 취소와 실제 정지 확인.
5. 수동 조작 패드를 끌거나 방향키로 움직여 보고, 손을 떼면 멈추는지, 벽 앞에서 충돌 감시가 멈추는지 확인한다.
6. **구역 편집**으로 진입 금지 구역을 저장 → 주행 반영 `APPLIED` → 목적지 이동이 피해 가는지 확인.
7. 디버깅의 **진단 실행**으로 발행자 없는 토픽·준비 안 된 Action이 없는지 본다.
8. 카메라 라이브 영상 확인. 연결 끊김이나 브라우저 종료를 정지 수단으로 사용하지 않는다.
9. `speech_preflight_passed` → `speech_peers_ready` 확인 후 마이크에 말하고 스피커 응답을 확인한다.
