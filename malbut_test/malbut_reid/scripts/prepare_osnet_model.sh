...
if [[ ! -x "$export_env/bin/python" ]] || \
  ! "$export_env/bin/python" -m pip --version >/dev/null 2>&1; then
  python3 -m venv "$export_env" || {
    echo 'Install python3-venv, then rerun this script.' >&2
    exit 1
  }
  "$export_env/bin/python" -m pip install --upgrade pip
fi
if ! "$export_env/bin/python" -c 'import torch, onnx'; then
  "$export_env/bin/python" -m pip install \
    --index-url https://download.pytorch.org/whl/cpu \
    'torch==2.5.1'
  "$export_env/bin/python" -m pip install 'onnx==1.16.2'
fi
