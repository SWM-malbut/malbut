...
서버 API·DB 변경·배포 순서는 `malbut_web/docs/fall_settings.md`에 정리했다.
...
- VLM은 통합 navigation Bringup의 준비 완료 뒤 실행한다. 최신 Bringup에는 별도 센서 모드나 `mode` 인자가 없다.
...
아래는 추가한 부분만 보인 예시다. 서버 DB에 `0011_fall_settings`가 적용되어야 전달한다.
...
- `0011_fall_settings`에서 감지·동의를 기본 OFF로 두고 설정 번호·변경 이력·회신 이력을 저장한다.
