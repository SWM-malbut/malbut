...
- 소유자만 저장할 수 있는 기존 웹 설정 API의 권한 검사를 유지한다.
  낙상 감지·Cloud 동의 저장 필드와 설정 번호의 서버 저장은 별도로 확장해야 한다.
...
- 설정 적용 목표 3초·회신 없음 6초, 양방향 확인 1초·끊김 판단 5초, Cloud 응답 20초는 확정이다.
  **서버 설정 미확인 시 Cloud 중단 기준도 15초로 확정했다.** 서버 응답 확장은 3.6절의 제안이다.
...
- 홈캠–Manager 설정 전달·적용 회신 형식은 작성했다. 현재 실행을 확인하는 연결과 ROS 발행 권한,
  웹의 실제 저장 필드·마이그레이션·실행 상태 전달은 구현 담당자와 맞추고 함께 검증해야 한다.
