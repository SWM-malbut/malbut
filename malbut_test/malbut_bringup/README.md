...
AutoSLAM 서버와 함께 음성 점검을 시작한다. 의존성 점검 → Agent·TTS → ROS 연결
확인 → 실제 STT 모델 로딩 1회 순서로 시작한다. 시작 중 CUDA 메모리 부족은 5초, 10초 대기 후 최대
3회 시도하며, 재시도 소진·제한시간 초과 또는 다른 음성 구성 실패 시 Bringup 전체를 종료한다.
