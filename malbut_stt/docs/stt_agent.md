...
- `check_only=true`는 상태를 변경하지 않고 해당 ID가 현재 활성 세션인지 조회한다. Agent는 무응답 확정 직전에 조회하여 STT 재시작 등으로 청취 세션이 사라진 경우를 사용자 무응답과 구분한다.
...
- 최종 전사는 기존 `/malbut/speech/transcript`에 `session_id`를 포함해 발행한다. 일반 대화 전사의 `session_id`는 빈 값이다. 최종 전사 실패 또는 청취 중단은 해당 세션의 `FAILED` 입력 상태로 알린다.
