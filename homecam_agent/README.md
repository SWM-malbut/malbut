...
Gazebo 실행 스크립트는 해당 사용자 캐시 환경을 자동 연결하며, 별도 위치를
쓰면 `HOMECAM_ONNX_RUNTIME_SITE_PACKAGES`로 지정한다. 아래 실제 로봇 서비스는
...
installer는 제조사 `install/setup.bash`를 먼저 읽고, 이후
`install/malbut_test/local_setup.bash`가 있으면 배포 overlay로 선택한다.
없으면 일반 `install/local_setup.bash`를 사용한다. 별도 설치 위치는
`--overlay /absolute/path/to/install` 또는 해당 `local_setup.bash` 파일 경로로
지정한다. 따라서 `malbut_test` 배포는 기존 제조사 환경 위에 빌드한 홈캠 패키지를
찾으며, 별도 overlay가 없는 일반 ROS workspace도 그대로 사용할 수 있다.
...
HOMECAM_IMAGE_TOPIC=/depth_cam/rgb0/image_raw
HOMECAM_CAMERA_INFO_TOPIC=/depth_cam/rgb0/camera_info
```

서비스는 `homecam_robot.launch.py`로 미디어만 실행한다. 공용 인식과 카메라
드라이버는 기존 로봇 Bringup에서 관리한다. 서비스는 부팅 시
자동 시작하고 장애 종료 시 재시작합니다. 첫 heartbeat의 desired state를 받기
전에는 기존 fail-closed 계약에 따라 이벤트 감지와 Storage 업로드를 시작하지
않습니다.

Depth 자세 확인은 별도로 `homecam_aurora.launch.py`의 detector를 실행할 때만
사용한다. `depth_image_topic`, `depth_camera_info_topic`, `depth_aligned_to_rgb`,
`depth_scale_m`, `depth_max_stamp_delta_sec`, `camera_height_m`, `camera_pitch_rad`를
launch 인자로 전달한다. RGB 정렬과 실제 장착 높이·각도를 확인하기 전에는
`depth_aligned_to_rgb:=false`를 유지한다. 위 미디어 전용 systemd 서비스는
이 설정으로 detector나 카메라 드라이버를 추가 실행하지 않는다.
