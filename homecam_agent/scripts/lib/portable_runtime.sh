...
homecam_config_key_allowed() {
  case "$1" in
    ...
    HOMECAM_CAMERA_HEIGHT_M|\
    HOMECAM_CAMERA_PITCH_RAD|\
    HOMECAM_CLOUD_MAP_ENABLED|\
    HOMECAM_DEVICE_ID|\
    HOMECAM_DEVICE_TOKEN_FILE|\
    HOMECAM_DEPTH_ALIGNED_TO_RGB|\
    HOMECAM_DEPTH_CAMERA_INFO_TOPIC|\
    HOMECAM_DEPTH_IMAGE_TOPIC|\
    HOMECAM_DEPTH_MAX_STAMP_DELTA_SEC|\
    HOMECAM_DEPTH_SCALE_M|\
