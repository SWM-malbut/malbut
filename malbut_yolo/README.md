...
```bash
ros2 launch malbut_yolo yolo.launch.py use_sim_time:=true
```
...
`prepare_yolo26_model.sh`는 기존 홈캠 ONNX 소비자용으로 보존된 별도 준비 도구다.
새 공유 검출기는 그 ONNX 경로를 쓰지 않는다. 기존 `malbut_perception` 모델 캐시
