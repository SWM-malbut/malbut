...
OpenAI 운영 구성의 적격 일반 대화는 C 방식으로 처리한다. 첫 호출은 기억
후보 없이 답변을 만들고, 답변과 원문 job을 함께 확정한 뒤 worker가 별도
추출·필요한 검토·저장을 수행한다. 명시적인 기억 관리 요청은 동기 처리한다.
...
sequenceDiagram
    ...
    Caller->>Agent: HTTP 또는 STT 발화
    Agent->>Provider: answer_only 대화 호출
    Provider-->>Agent: 기억 제안 없는 답변
    Agent->>SQLite: 답변과 원문 job 확정
    Agent-->>Caller: 고정된 응답 반환
    MemoryWorker->>SQLite: job claim과 유효성 검사
    MemoryWorker->>Provider: 현재 원문만 별도 추출
    Provider-->>MemoryWorker: 미검증 후보 또는 null
    MemoryWorker->>SQLite: 후속 호출 전 유효성 재검사
    MemoryWorker->>Provider: 필요한 후보만 의미 검토
    Provider-->>MemoryWorker: 후보 승인 또는 거절
    MemoryWorker->>SQLite: 재검사 후 기억과 의존 관계 저장
```

그림은 검토가 필요한 경로를 요약한다. 추출 결과가 `null`이면 검토·저장을
생략한다. 실제 HTTP 송신·음성 발행 전에 worker가 끝날 수도 있지만 호출자는
추출·검토 완료를 기다리지 않고 확정된 답변을 받는다.
...
| `나는 커피를 좋아해` | 동의 이후 선호를 자동 저장 대상으로 처리. 운영 비동기 경로는 저장 완료를 기다리지 않음 |
...
추측은 자동 저장하지 않는다. 명시적 관리 요청의 모호한 대상·충돌은 질문하며,
자동 후보의 거절·충돌은 기존 기억과 답변을 유지하고 질문을 만들지 않는다.
기억 관리와 로봇 실행 확인이 겹친 단순한 `네`로 로봇을 승인하지 않는다.
명시적 요청은 기억 변경과 최종 답변을 같은 DB 트랜잭션으로 확정한다.
자동 경로는 답변과 private job을 먼저 함께 확정하고 기억은 이후 별도로
확정한다. 어느 경로든 모델 호출 동안 DB 트랜잭션을 잡지 않는다.
...
늦게 저장된 자동 사실은 출처뿐 아니라 같은 세션 instance·generation의 완료된
후속 턴에도 의존 관계를 연결한다. 추론 중인 후속 턴은 최종 확정 시 최신 의존
관계를 다시 읽어 합친다. 답변을 다시 쓰지 않으면서 이후 삭제·정정 시 관련
원문과 답변을 새 모델 입력에서 제외하기 위한 처리다.

OpenAI factory의 자동 경로만 `answer_only`와 별도 source-only 추출을 사용한다.
명시적 저장·조회·정정·삭제·동의와 보류 확인은 기존 동기 경로를 유지한다.
RAI는 답변·후보 통합 호출과 적격 후보의 비동기 검토·저장을 사용한다. RAI 기억
경로는 내부 프로토콜 v2를 요구하며 v1 호출 계약은 유지한다. v2를 지원하지
않는 sidecar의 결과를 기억 처리 성공으로 안내하지 않는다. Mock은 기존 동기
처리와 개발 시험용 고정 한국어 패턴을 유지하며 실제 모델의 한국어 해석 성능을
보장하지 않는다. 직접 생성한 `AgentOrchestrator`도 별도 설정 없이는 기존
동기 처리가 기본이다.

마이그레이션은 기존 대화·기억 DB를 열 때 필요한 테이블과 누락된 메타데이터·
큐 순번 컬럼을 추가하는 방식이다. 기억 정책·사실 구분·삭제 출처·응답 의존
관계·영속 job을 추가하고 기존 기록은 보존한다. 비동기 추출·검토의 시간과 알려진
토큰 사용량은 각각 `extraction_*`, `review_*`로 기록하며 공개 응답의 최초
대화 메타데이터에 소급 합산하지 않는다.
...
개인화 설정 요청이나 저장 금지 등 부정형 기억 관리 표현은 요청 예약 시 같은
사용자의 이전 job을 원자적으로 취소하고 무효화 revision을 올린다. 삭제 대상이
없어도 적용하여 과거 추론 결과의 뒤늦은 저장을 막는다. 검증된 지연 자동 추가
저장만 이 사용자별 revision을 유지하며, 저장소 전체 mutation counter는 실제
추가 시 증가한다. 따라서 일반 대화와 후속 자동 저장을 불필요하게 무효화하지
않으면서 명시적인 변경 의도는 우선한다.

자동 job은 같은 사용자 안에서 큐 등록 순서대로 처리한다. 재시작 후 남은
`running` job은 인수·재전송하지 않으며, 만료까지 같은 사용자의 후속 작업만
claim을 기다리게 한다. 다른 사용자의 적격 작업은 진행할 수 있다. 세션 초기화·
삭제·만료, 명시적 관리 요청, TTL·큐 한도·충돌·오류로 저장이 생략될 수 있다.
답변이 돌아왔거나 대화 문맥에서 해당 사실을 활용한다고 영속 저장까지 완료된
것은 아니며 자동 저장 완료를 안내하지 않는다.

OpenAI C는 적격 인사·일반 대화에서도 별도 추출이 `null`을 반환하기까지 호출
비용이 생길 수 있다. 추가 추출·검토를 foreground 대기에서 분리하지만 총비용
...
영속 큐·취소·재시작·실제 응답 전달 검증을 대신하지 않는다.
...
- 최종 음성 전사는 한 번에 최대 16,000자를 접수한다. 전체 원문과 끝부분을
  ...
  긴 음성은 짧은 입력용 Front Router를 건너뛰고 기존 Provider에 전달하며,
  prompt에는 늘어난 원문 길이와 JSON escape에 필요한 공간만 추가한다.
