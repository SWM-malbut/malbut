...
VLM 쪽은 구현했고, 실제 설정을 보내는 웹·홈캠·Manager 연결은 남아 있다.
...
다만 상위 카메라·YOLO 노드의 발행 조건 분리와 Manager 설정 전달까지 끝났다는 뜻은 아니다.
Manager ID를 전달하는 자동화는 아직 없어 기본 Bringup 실행은 대기한다.
...
서버 확인 시각을 전달하는 Manager 발행부와 기존 서버 응답의 `fallSettings` 확장은
[호출 명세 3.5~3.6절](fall_manager_contract.md)에 적었으며 아직 구현하지 않았다.
...
VLM Service 서버는 구현했고 서버 필드·홈캠·Manager 연결은 남아 있다.
...
설정·상태 연결은 다음과 같다. Manager 쪽 발행·수신은 별도 구현해야 한다.
