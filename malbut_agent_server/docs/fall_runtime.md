...
VLM·홈캠·Manager의 로봇 안 연결은 구현했다. 서버의 낙상 설정 응답과 웹 저장·표시는 남아 있다.
...
로봇의 설정 전달·적용 회신은 구현했다. 서버와 웹의 저장·표시는 아직 구현하지 않았다.
...
서버 확인 시각을 전달하는 Manager 발행부는 구현했다. 서버 응답의 `fallSettings` 확장은
[호출 명세 3.5~3.6절](fall_manager_contract.md)의 제안이며 서버 쪽은 아직 구현하지 않았다.
