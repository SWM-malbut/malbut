...
| 웹 감지·Cloud 동의 설정, 저장 후 로봇 적용·결과 표시 | 로봇의 설정 전달·적용 회신 구현. 웹 UI/API·서버 저장·표시는 미구현 |
| 홈캠–Manager 설정 전달·적용 회신 | Topic 송수신과 heartbeat 요청 포함 구현. 서버의 새 필드 처리는 남음 |
...
| 서버 설정 미확인 시 Cloud 중단 | 홈캠/Manager의 원래 확인 시각 전달·VLM 15초 차단 구현. 서버 응답 확장은 남음 |
