...
```bash
ros2 launch malbut_reid person_reidentification.launch.py use_sim_time:=true
```

실제 로봇에서는 `use_sim_time:=false`를 사용한다. 같은 카메라를 사용하는
...
준비 스크립트는 사용자가 명시적으로 실행하는 설치/변환 도구다. 노드 시작 시
다운로드하거나 ROS 설치를 변경하지 않는다. 모델 변환용 환경은 cache 안에
분리한다. 기존 x86 NVIDIA/JetPack 6 ARM64 ONNX Runtime 설치 경로와
TensorRT/CUDA/CPU 실행 선택을 유지한다. Jetson에서는 JetPack/L4T와 wheel
호환성을 확인하고 해당 로봇에서 TensorRT 캐시를 생성해야 한다.
