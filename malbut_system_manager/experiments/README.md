...
버튼으로 직접 실험하려면 다음 한 줄을 실행한다. 자동 명령이나 시간 종료 없이
Gazebo·RViz·카메라와 조작 창을 띄운다. 순찰, 사람 추적, 좌표 이동, 전체 중지를
사용할 수 있다. 모든 명령은 실제 매니저를 거치며, 전체 중지는 보류 미션도
취소한다. 조작 창을 닫으면 이 실행의 모든 프로세스를 종료한다.

```bash
bash malbut_system_manager/experiments/run_mission_sequence.sh --interactive
```

GUI는 Python 표준 Tkinter를 사용한다(Ubuntu 패키지 `python3-tk`).
...
4. 목적지 이동 요청 → 같은 NORMAL 우선순위인 기존 순찰이 하위 실행까지 종료되고 상위 요청도 교체 사유와 함께 끝나는지 확인 → 12초 관찰.
5. 앞선 이동이 이미 끝났다면 정지·IDLE 확인 후 순찰을 **새 요청으로** 시작하고 12초 관찰한다. 자동 재개에 의존하지 않는다.
6. 사람 추적 요청 → 앞선 이동 또는 새 순찰의 완전 취소와 실제 `/follow_person` 실행 확인 → 12초 관찰.
7. 사람 추적 취소 → 관리자 `IDLE`, 활성·대기·보류 목록 비움, 하위 Action 종료, 오도메트리 기준 3초 정지 확인 → 12초 동안 기존 미션이 재개되지 않는지 확인. 교체 중 `SUSPENDED`로 넘어가거나 예상하지 않은 실행 실패가 발생하면 실패한다.

직접 취소한 요청은 `CANCELED`, 관리자가 교체한 요청은 `ABORTED`와 정확한
`message: mission preempted by a replacement request`를 확인한다. 교체 대상이
아닌 요청의 `ABORTED`나 다른 실패 사유는 통과시키지 않는다.
...
**관리자 통과와 응용 성능을 구분한다.** 이 실험은 실제 기능의 실행·선점·완전 취소와 자동 재개 방지를 확인하는 smoke test다. 순찰 전체 완료, 카메라의 실제 공간 관측률, 사람 추적 정확도를 합격시켰다는 의미가 아니다. `target_visible` 피드백과 검출 토픽 수신도 따로 기록하므로 추적 서버 실행만으로 사람을 따라갔다고 판단하지 않는다. 이동 거리는 오도메트리 기반이며 Gazebo ground truth가 아니다.
