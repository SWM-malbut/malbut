...
import { createDeploymentSource } from "./deployment-source";
...
export interface HomecamDevStackProps extends StackProps {
  ...
  readonly ingressMode?: "custom-domain" | "cloudfront";
  readonly deploymentSourceDirectory?: string;
}
...
export class HomecamDevStack extends Stack {
  constructor(scope: Construct, id: string, props: HomecamDevStackProps) {
    ...
    const ingressMode = props.ingressMode ?? "custom-domain";
    if (!["custom-domain", "cloudfront"].includes(ingressMode)) {
      throw new Error("ingressMode must be custom-domain or cloudfront");
    }
    const usesCloudFront = ingressMode === "cloudfront";
    if (usesCloudFront && props.authMigrationPhase !== "cleanup") {
      throw new Error("cloudfront ingress requires authMigrationPhase=cleanup");
    }
    ...
    const parameters = deploymentParameters(this, usesCloudFront);
    const homecamDomainName = parameters.hostedZoneName?.valueAsString ?? "";
    const hostedZone = usesCloudFront ? undefined : route53.HostedZone.fromHostedZoneAttributes(
      this,
      "HomecamHostedZone",
      {
        hostedZoneId: parameters.hostedZoneId!.valueAsString,
        zoneName: homecamDomainName,
      },
    );
    const certificate = hostedZone ? new acm.Certificate(this, "HomecamCertificate", {
    ...
    const sourceAsset = createDeploymentSource(this, props.deploymentSourceDirectory);
    const imageTag = sourceAsset ? `snapshot-${sourceAsset.assetHash}` : props.containerImageTag;
    const imageTagVariable = sourceAsset ? "$IMAGE_TAG" : "$GIT_SHA";
    const buildDirectory = sourceAsset ? '"$CODEBUILD_SRC_DIR"' : "/tmp/malbut-source/malbut_web";
    const imageBuilder = new codebuild.Project(this, "HomecamImageBuilder", {
      ...
      ...(sourceAsset ? {
        source: codebuild.Source.s3({ bucket: sourceAsset.bucket, path: sourceAsset.s3ObjectKey }),
      } : {}),
      buildSpec: codebuild.BuildSpec.fromObjectToYaml({
        ...
        phases: {
          ...
          build: {
            commands: [
              `docker build --pull --platform linux/arm64 -t "${repository.repositoryUri}:${imageTagVariable}" ${buildDirectory}`,
              `docker image inspect --format '{{.Architecture}}' "${repository.repositoryUri}:${imageTagVariable}" | grep -qx arm64`,
            ],
          },
          post_build: {
            commands: [`docker push "${repository.repositoryUri}:${imageTagVariable}"`],
          },
        },
      }),
      environment: {
        ...
        environmentVariables: {
          ...(sourceAsset ? { IMAGE_TAG: { value: imageTag } } : { GIT_SHA: { value: props.containerImageTag } }),
        },
      },
    ...
    const container = taskDefinition.addContainer("HomecamWeb", {
      // Deployed ECS/ALB identifier: renaming it would re-map the running service.
    ...
    const privateLoadBalancer = usesCloudFront
      ? new elbv2.ApplicationLoadBalancer(this, "HomecamPrivateLoadBalancer", {
          vpc,
    ...
    const service = new ecsPatterns.ApplicationLoadBalancedFargateService(
      ...
      "HomecamService",
      {
        ...
        ...(privateLoadBalancer ? { loadBalancer: privateLoadBalancer } : { publicLoadBalancer: true }),
        openListener: !usesCloudFront,
        circuitBreaker: { rollback: true },
        minHealthyPercent: 100,
        certificate,
        protocol: usesCloudFront ? elbv2.ApplicationProtocol.HTTP : elbv2.ApplicationProtocol.HTTPS,
        listenerPort: usesCloudFront ? 80 : 443,
        redirectHTTP: !usesCloudFront,
    ...
    let publicOrigin = Fn.join("", ["https://", homecamDomainName]);
    if (usesCloudFront) {
      // The ALB is private: HTTP is confined to the VPC origin connection, not
      // exposed as an unencrypted public route around CloudFront.
      const vpcOrigin = new cloudfront.VpcOrigin(this, "HomecamVpcOrigin", {
        endpoint: cloudfront.VpcOriginEndpoint.applicationLoadBalancer(service.loadBalancer),
        protocolPolicy: cloudfront.OriginProtocolPolicy.HTTP_ONLY,
      });
      vpcOrigin.node.addDependency(service.listener);
      const distribution = new cloudfront.Distribution(this, "HomecamDistribution", {
        comment: `${prefix} authenticated application`,
        defaultBehavior: {
        ...
        errorResponses: [400, 403, 404, 405, 414, 416, 500, 501, 502, 503, 504].map(
          (httpStatus) => ({ httpStatus, ttl: Duration.seconds(0) }),
        ),
      });
      // Query the regional AWS-managed prefix list at deployment, so offline
      // synth does not require cached account-specific context or a hardcoded ID.
      const cloudfrontPrefixList = new cr.AwsCustomResource(this, "CloudFrontPrefixList", {
        ...
        onUpdate: {
          ...
          physicalResourceId: cr.PhysicalResourceId.of(`cloudfront-origin-facing-${this.region}`),
        },
        policy: cr.AwsCustomResourcePolicy.fromStatements([
          new iam.PolicyStatement({ actions: ["ec2:DescribeManagedPrefixLists"], resources: ["*"] }),
        ]),
      });
      const originIngress = new ec2.CfnSecurityGroupIngress(this, "CloudFrontOriginIngress", {
        groupId: service.loadBalancer.connections.securityGroups[0]!.securityGroupId,
        ipProtocol: "tcp",
        fromPort: 80,
        toPort: 80,
        sourcePrefixListId: cloudfrontPrefixList.getResponseField("PrefixLists.0.PrefixListId"),
        description: "Only CloudFront origin-facing traffic may reach the private ALB",
      });
      vpcOrigin.node.addDependency(originIngress);
      // CloudFront appends the actual viewer address. Do not append the
      // CloudFront connection address again: login rate limiting uses the last IP.
      service.loadBalancer.setAttribute("routing.http.xff_header_processing.mode", "preserve");
      service.loadBalancer.setAttribute("routing.http.preserve_host_header.enabled", "true");
      publicOrigin = Fn.join("", ["https://", distribution.distributionDomainName]);
      container.addEnvironment("AUTH_PUBLIC_ORIGIN", publicOrigin);
    ...
    new CfnOutput(this, "ContainerImageTag", { value: imageTag });
    if (sourceAsset) {
      new CfnOutput(this, "DeploymentSourceHash", { value: sourceAsset.assetHash });
    }
...
function deploymentParameters(stack: Stack, usesCloudFront: boolean) {
  return {
    hostedZoneId: usesCloudFront ? undefined : new CfnParameter(stack, "HomecamHostedZoneId", {
      type: "String",
      description: "Existing Route 53 public child hosted zone ID",
      allowedPattern: "^Z[A-Z0-9]{1,31}$",
    }),
    hostedZoneName: usesCloudFront ? undefined : new CfnParameter(stack, "HomecamHostedZoneName", {
      type: "String",
      description:
