...
def test_narrow_doorway_creates_two_automatic_room_candidates(tmp_path):
    """A wall with a narrow doorway must seed two room candidates."""
    map_path = _write_slam_map(
        tmp_path,
        "two_rooms",
        [(20, 20), (190, 20), (190, 130), (20, 130)],
    ...
    assert user_map["room_segmentation"]["room_count"] == 2
    assert len(rooms) == 2
    assert all(room["properties"]["generated"] for room in rooms)
    assert sum(room["properties"]["area_m2"] for room in rooms) > 40.0
...
def test_editor_loads_maps_dynamically_and_has_no_fixed_house_geometry():
    ...
    assert "function validateZoneRing(ring, room)" in script
    assert "function reconcileZoneRooms(zones = state.zones)" in script
