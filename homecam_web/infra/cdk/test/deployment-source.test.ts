...
test("deployment sources stay inside the repository and contain the build entrypoints", () => {
  ...
  try {
    const source = path.join(root, "malbut_test/homecam_web");
    ...
    assert.equal(resolveDeploymentSource("malbut_test/homecam_web", root), source);
    for (const candidate of ["", "/tmp", "../outside", "malbut_test/../homecam_web", "."]) {
    ...
    assert.throws(() => resolveDeploymentSource("malbut_test/homecam_web", root), /regular Dockerfile/);
  } finally {
