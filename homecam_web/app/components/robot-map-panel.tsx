...
export function RobotMapPanel({
  ...
  const [managedGoal, setManagedGoal] = useState<{ x: number; y: number; deviceId: string; mapId: string } | null>(null);
  ...
  const sendCommand = async (operation: RobotOperation, commandPayload: Record<string, unknown> = {}) => {
    ...
    try {
      ...
      setNotice(
        ["runtime_start", "runtime_stop", "mission_start", "mission_cancel"].includes(operation)
      ...
      <div className="robot-map-topbar">
        ...
        <div className="robot-map-mode-tabs" aria-label="지도 모드">
          {([
            ...
            ["rooms", "방 편집"],
            ["zones", "구역 편집"],
          ] as Array<[MapMode, string]>).filter(([mode]) => !managed || mode === "view" || mode === "navigate").map(([mode, label]) => (
            <button
              key={mode}
              type="button"
